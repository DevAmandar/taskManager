...
type Props = PropsWithChildren<{
    editModalRef: RefObject<HTMLDialogElement | null>
    boardIndex: number
    listIndex: number
}>

export default function EditTitleListModal({ editModalRef, boardIndex, listIndex }: Props) : ReactNode {
    ...
    const handleSubmit = (e: FormEvent<HTMLFormElement>) => {
        ...
        if (validation(title)) {
            dispatchLists({ type: 'rename_list_title', boardIndex: boardIndex,listIndex:listIndex ,title: title })
            formRef.current?.reset()
            editModalRef.current?.close()
            setShowError(null)
        }
